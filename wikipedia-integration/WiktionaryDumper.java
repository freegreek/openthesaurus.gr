 ...
 * Contains some filtering that's specific the Greek.
 *
 * Get the XML dump from http://download.wikimedia.org/elwiktionary/latest/,
 * the filename is something like "XXwiktionary-YYYYMMDD-pages-articles.xml.bz2",
 * whereas XX is the language code (de, en, fr,el, etc).
 *
 * @author Daniel Naber
 * adapted to Greek wiktionary from Vasilis Kasimatis
 */
public class WiktionaryDumper {

    /** Lines matching this string indicate the "meanings" section in a Wiktionary page. */

    private static final String MEANINGS_PATTERN="===\\{\\{[^\\|]+\\|el\\}\\}===";

    /** Lines matching this string indicate the "synonyms" section in a Wiktionary page. */
    private static final String SYNONYMS_PATTERN = "====\\{\\{συνώνυμα\\}\\}====";

    /** Lines matching this string indicate undefined meaning for the synonym */
    private static final String UNDEFINED_SYNONYM="{{λείπει ο ορισμός}}";

    /** String required in a document, other documents will be ignored. */
    private static final String LANGUAGE_STRING = "{{-el-}}";
    ...
    /** The following patern indicates a valid meaning for the synonym */
    private final Pattern SYNONYMS_PREFIX_PATTERN = Pattern.compile("\\* *");

    /** The following patern indicates a specific category this synonym belongs to, e.g. "{{μτφρ}}" ,"{{ζωολ}}" etc.  */
            ...
            if (qName.equals("title")) {
                ...
                if (title.length() > 0) {
                    if (text.indexOf(LANGUAGE_STRING) == -1) {
                        ...
                        final List<String> meaningsList = getSection(cleanedText, MEANINGS_PATTERN);
                        final String meanings = join(meaningsList, " ");
                        final List<String> synonymsList = getSection(cleanedText, SYNONYMS_PATTERN);
        ...
        private List<String> getSection(final String text, final String prefix) {
            ...
            while (scanner.hasNextLine()) {
                ...
                if (line.trim().matches(prefix)) {
                    ...
                    if(
                        line.indexOf(UNDEFINED_SYNONYM)<0 &&
                        SYNONYMS_PREFIX_PATTERN.matcher(line).lookingAt()
                    ) {

                        String sText=SYNONYMS_PREFIX_PATTERN.matcher(line).replaceFirst(":["+(terms.size()+1)+"] ");
                        sText=CATEGORY_PATTERN.matcher(sText).replaceAll("[[$1]]:");
